...
import { settingsActions, settingsStore } from './settings/settingsStore';
...
describe('front-end components', () => {
  ...
  it('updates every settings group and confirms reset', async () => {
    ...
    const textInputs = screen.getAllByRole('combobox');
    await user.clear(textInputs[0]);
    await user.type(textInputs[0], 'Arial');
    ...
    await user.selectOptions(textInputs[3], 'full');
  ...
  it('updates remaining settings fields and leaves values when reset is cancelled', async () => {
    ...
    await user.clear(combos[1]);
    await user.type(combos[1], 'Editor Font');
    await user.clear(combos[2]);
    await user.type(combos[2], 'Code Font');
    await user.selectOptions(combos[3], '1200');
    await user.click(screen.getByRole('button', { name: 'Reset to Defaults' }));
